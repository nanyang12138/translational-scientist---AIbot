...
with ZipFile(out, "w", ZIP_DEFLATED) as z:
    ...
    z.writestr(
        ...
        ")\r\n"
        "if \"%LLM_PROVIDER%\"==\"\" (\r\n"
        "  echo Choose provider: amd-openai, gateway, azure, openai, ollama, mock\r\n"
        "  set /p LLM_PROVIDER=LLM_PROVIDER [amd-openai]: \r\n"
        "  if \"%LLM_PROVIDER%\"==\"\" set LLM_PROVIDER=amd-openai\r\n"
        ")\r\n"
        "if /I \"%LLM_PROVIDER%\"==\"amd-openai\" (\r\n"
        "  if \"%AMD_LLM_GATEWAY_KEY%\"==\"\" set /p AMD_LLM_GATEWAY_KEY=AMD_LLM_GATEWAY_KEY / subscription key: \r\n"
        "  if \"%LLM_GATEWAY_USER%\"==\"\" set /p LLM_GATEWAY_USER=user header value: \r\n"
        "  if \"%LLM_MODEL%\"==\"\" set LLM_MODEL=gpt-5.5\r\n"
        ")\r\n"
        ...
        "  if \"%LLM_GATEWAY_USER%\"==\"\" set /p LLM_GATEWAY_USER=user header value: \r\n"
        "  if \"%LLM_USE_DEPLOYMENT_PATH%\"==\"\" set /p LLM_USE_DEPLOYMENT_PATH=Use /openai/deployments/{model}/chat/completions? 1=yes blank=no: \r\n"
    ...
    z.writestr(
        ...
        "2. Double-click \"Start Agent Bridge.bat\" and choose provider: amd-openai, gateway, azure, openai, ollama, or mock.\\n"
        "3. Launch the game, open Settings, enable LLM Agent mode, keep endpoint http://127.0.0.1:8787/resolve.\\n\\n"
        "AMD Hermes/OpenAI gateway example variables:\\n"
        "LLM_PROVIDER=amd-openai\\n"
        "AMD_LLM_GATEWAY_KEY=<your-subscription-key>\\n"
        "LLM_GATEWAY_USER=<your-user>\\n"
        "Defaults: LLM_BASE_URL=https://llm-api.amd.com/OpenAI, LLM_MODEL=gpt-5.5, LLM_API_KEY=placeholder-key\\n\\n"
        "Custom gateway example variables:\\n"
        ...
        "Deployment-style gateway:\\n"
        "LLM_USE_DEPLOYMENT_PATH=1 -> <base>/openai/deployments/<model>/chat/completions\\n"
